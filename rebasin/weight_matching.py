...
from rebasin.utils import rngmix


class PermutationSpec(NamedTuple):
  """
    str (names of distinct permutations): List[Tuple[str (name of layer with perm), int (dim with this perm)]]
  """
  perm_to_axes: dict
  """
    str (layer name): Tuple[ (for each dim in layer shape) Union[None, str (name of permutation)]]
  """
  axes_to_perm: dict

  @staticmethod
  def from_axes_to_perm(axes_to_perm: dict):
    return permutation_spec_from_axes_to_perm(axes_to_perm)

  def sizes(self, state_dict):
    dims = {}
    for p, axes in self.perm_to_axes.items():
      dims[p] = state_dict[axes[0][0]].shape[axes[0][1]]
      for layer_name, dim in axes:
        assert state_dict[layer_name].shape[dim] == dims[p]
    return dims


class PartialPermutationSpec(PermutationSpec, NamedTuple):
  perm_to_target_size: dict
...
def weight_matching(rng,
  ...
  for iteration in range(max_iter):
    progress = False
    # randomly choose order in which to solve permutations
    for p_ix in random.permutation(rngmix(rng, iteration), len(perm_names)):
      ...
      for wk, axis in ps.perm_to_axes[p]:
        ...
        A += w_a @ w_b.T  # identical vectors maximize dot product
