...
parser.add_argument('--save_dir', default="outputs/exp_1", type=Path)
...
# experiment
save_dir = args.save_dir / f"{args.train_data}_{args.loss}_{args.bias_loss_weight}_{args.ckpt}"
save_dir.mkdir(parents=True, exist_ok=True)
for i in range(1, args.n_replicates + 1):
    for j in range(1, i):
        save_file = save_dir / f"errors_{i}_{j}_{args.level}.pt"
        if args.overwrite or not save_file.exists():
            ...
            values = align_and_error(model, state_dict_f, state_dict_g, dataloader,
                                args.barrier_resolution, args.loss, args.bias_loss_weight)
            torch.save(values, save_file)
        else:
            print(f"Skipping already created {save_file}")
