...
from nnperm_utils import evaluate_per_sample, multiplicative_weight_noise, add_skip_weights_to_open_lth_resnet, infer_open_lth_computation_graph
...
class TestPermuteNN(unittest.TestCase):
    ...
    def test_compute_graph(self):
        for model, data in self.conv_models:
            graph = infer_open_lth_computation_graph(next(iter(data))[0].shape, model.state_dict())
            print(graph)
