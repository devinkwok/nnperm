...
#SBATCH --output=narrow-wide-barrier-%j.out
#SBATCH --error=narrow-wide-barrier-%j.err

source ./open_lth/slurm-setup.sh cifar10

CKPT_ROOT=$HOME/scratch/open_lth_data/
BARRIER_ROOT=$HOME/scratch/2022-nnperm/narrow-wide
...
parallel --delay=15 --linebuffer --jobs=1  \
    python -m scripts.open_lth_barriers  \
        --repdir_a=$CKPT_ROOT/{3}/replicate_{5}  \
        --repdir_b=$CKPT_ROOT/{3}/replicate_{6}  \
        ...
        --train_ep_it=$EPOCH  \
        --levels=$LEVEL  \
        --save_file=$BARRIER_ROOT/{1}"/"{2}"-"{4}"/"{3}"/"{5}"-"{6}"/barrier-level_0-main-ep160_it0.pt"  \
